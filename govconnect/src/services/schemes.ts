...
let schemesStore: Scheme[] = mockSchemes.map((s) => ({ ...s }));
...
export interface SchemeFilters {
  ...
  ministry?: string;
  state?: string; // 'central' | state name
}
 ...
 */
export const getSchemes = async (filters?: SchemeFilters): Promise<Scheme[]> => {
  await delay(400);
  ...
  if (filters?.state && filters.state.trim() !== '') {
    if (filters.state === 'central') {
      ...
      results = results.filter(
        (s) => !s.state || s.state.toLowerCase() === filters.state!.toLowerCase(),
      );
    }
  }
 ...
 */
export const getRecommendedSchemes = async (): Promise<Scheme[]> => {
  await delay(400);
 ...
 */
export const saveScheme = async (id: string): Promise<Scheme> => {
  await delay(200);
 ...
 */
export const getSavedSchemes = async (): Promise<Scheme[]> => {
  await delay(300);
