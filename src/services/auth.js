...
export const loginUser = async (credentials) => {
    ...
    try {
        ...
        const registeredUsers = registeredUsersJson
            ? JSON.parse(registeredUsersJson)
            : [];
        const found = registeredUsers.find((u) => (u.email.toLowerCase() === emailOrMobile.toLowerCase().trim() ||
...
export const updateProfile = async (data) => {
    ...
    const filledFields = completionFields.filter((f) => updatedUser[f] !== undefined && updatedUser[f] !== '' && updatedUser[f] !== 0).length;
