...
let grievancesStore: Grievance[] = mockGrievances.map((g) => ({ ...g }));
...
const PORTAL_MAP: Record<GrievanceCategory, OfficialPortal> = {
  public_services: {
 ...
 */
export const getGrievanceById = async (id: string): Promise<Grievance> => {
  await delay(300);
...
export const createGrievance = async (
  ...
  const newGrievance: Grievance = {
    ...
    status: 'draft' as GrievanceStatus,
    priority: 'medium',
    createdAt: now,
    updatedAt: now,
    timeline: [
...
export const analyzeGrievance = async (
  ...
  let priority: AiAnalysis['prioritySuggestion'] = 'medium';
  if (
  ...
  const reasoningMap: Record<GrievanceCategory, string> = {
    public_services:
      'Based on the complaint content, this falls under public service delivery issues handled by DARPG.',
    infrastructure:
      'The complaint relates to infrastructure – roads, bridges, or public works – under MoRTH jurisdiction.',
    corruption:
      'Corruption allegations are routed to the Central Vigilance Commission for impartial investigation.',
    education:
      'Education-related grievances are managed by the Ministry of Education through CPGRAMS.',
    health:
      'Health service complaints are addressed by the Ministry of Health & Family Welfare.',
    police:
      'Police-related complaints are handled at the state level by the Police Complaint Authority.',
    revenue:
      'Revenue and land record disputes fall under the State Revenue Department.',
    utilities:
      'Utility service failures (electricity/water) are addressed via INGRAM consumer helpline.',
    environment:
      'Environmental violations are investigated by MoEF&CC under the Environment Protection Act.',
    other:
      'This complaint has been routed to CPGRAMS, the central public grievance portal for general complaints.',
  };
