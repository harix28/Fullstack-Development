...
const normalize = (value) => value.toLowerCase().replace(/[^a-z0-9\u0900-\u097f₹\s-]/gi, ' ');
const hasAny = (text, words) => words.some((word) => text.includes(word));
const educationLabel = (education) => {
    const value = education?.toLowerCase() || '';
    if (value.includes('post') || value.includes('mca') || value.includes('master'))
        return 'Post Graduate / MCA';
    if (value.includes('graduate') || value.includes('bca') || value.includes('bachelor'))
        return 'Graduate';
    return education || 'Not specified';
};
const scoreScheme = (scheme, user = {}) => {
    let score = 45;
    const age = user.age;
    ...
    if (typeof age === 'number') {
        if (scheme.eligibility.minAge === undefined || age >= scheme.eligibility.minAge)
            score += 10;
        if (scheme.eligibility.maxAge === undefined || age <= scheme.eligibility.maxAge)
            score += 10;
    }
    if (typeof income === 'number' && scheme.eligibility.income !== undefined) {
        score += income <= scheme.eligibility.income ? 15 : -20;
    }
    if (scheme.eligibility.state?.some((s) => state.includes(s.toLowerCase())))
        score += 10;
    if (scheme.eligibility.category?.some((c) => category.includes(c.toLowerCase())))
        score += 5;
    if (scheme.eligibility.education?.some((e) => education.includes(e.toLowerCase())))
        score += 10;
    if (scheme.tags.some((tag) => education.includes(tag.toLowerCase()) || tag.toLowerCase().includes('education')))
        score += 5;
    return Math.max(20, Math.min(98, score));
};
const scoreJob = (job, user = {}) => {
    let score = 45;
    const education = user.education?.toLowerCase() || '';
    ...
    if (job.qualification.some((q) => {
        ...
        score += 20;
    if (job.tags.some((tag) => skills.some((skill) => tag.toLowerCase().includes(skill) || skill.includes(tag.toLowerCase()))))
        score += 15;
    if (job.state.toLowerCase() === 'central' || job.state.toLowerCase() === state)
        score += 8;
    if (typeof age === 'number' && age >= job.ageLimit.min && age <= job.ageLimit.max)
        score += 10;
    return Math.max(20, Math.min(98, score));
};
const schemeResults = (user) => mockSchemes
    .map((scheme) => ({ scheme, score: scoreScheme(scheme, user) }))
    .sort((a, b) => b.score - a.score)
    .slice(0, 3);
const jobResults = (user) => mockJobs
    .map((job) => ({ job, score: scoreJob(job, user) }))
    .sort((a, b) => b.score - a.score)
    .slice(0, 3);
const makeMessage = (response) => ({
    id: generateId(),
    role: 'assistant',
    timestamp: new Date().toISOString(),
    ...response,
});
export const sendMessage = async (message, history = [], currentContext) => {
    ...
    if (isGrievance) {
        ...
        if (hasAny(lower, ['garbage', 'kachra', 'kooda'])) {
            ...
            title = 'Garbage collection / sanitation issue';
            priority = 'high';
        }
        else if (hasAny(lower, ['paani', 'water supply', 'sewer', 'drain', 'waterlogging'])) {
            category = 'Water & Drainage';
            department = 'Local Water / Municipal Department';
            title = 'Water supply / drainage issue';
            priority = 'high';
        }
        else if (hasAny(lower, ['bijli', 'electricity', 'street light'])) {
            category = 'Electricity / Street Lighting';
            department = 'Electricity utility / Municipal authority';
            title = 'Electricity or street-light issue';
        }
        else if (hasAny(lower, ['road', 'sadak', 'pothole'])) {
            category = 'Road & Infrastructure';
            department = 'Municipal authority / PWD';
            title = 'Road / pothole repair request';
        }
    ...
    if (isJob) {
        ...
        const lines = results.map(({ job, score }) => `**${job.title}** — ${score}% profile match\n• ${job.organization} • ${job.location}\n• Qualification: ${job.qualification[0]}\n• Deadline: ${job.applicationDeadline}`);
        return makeMessage({
