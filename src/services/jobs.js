...
let jobsStore = mockJobs.map((j) => ({ ...j }));
// ─── Helpers ─────────────────────────────────────────────────────────────────
...
export const getJobs = async (filters) => {
    ...
    if (filters?.state && filters.state.trim() !== '') {
        results = results.filter((j) => j.state.toLowerCase() === filters.state.toLowerCase());
    }
    if (filters?.organization && filters.organization.trim() !== '') {
        const org = filters.organization.toLowerCase();
        results = results.filter((j) => j.organization.toLowerCase().includes(org));
    }
    return results;
};
/**
