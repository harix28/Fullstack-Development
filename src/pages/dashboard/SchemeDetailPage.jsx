...
import { ArrowLeft, CheckCircle2, Share2, Bookmark, ExternalLink, FileText, Calendar, HelpCircle, Shield, AlertTriangle } from 'lucide-react';
...
export default function SchemeDetailPage() {
    ...
    if (!scheme)
        return <div className="p-8">Scheme not found.</div>;
    ...
    return (<div className="space-y-6 max-w-7xl mx-auto font-sans pb-12">
      <Link to={ROUTES.SCHEMES} className="inline-flex items-center text-sm font-medium text-[#7D6E63] hover:text-[#59463B] transition-colors">
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 space-y-8">
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <Badge className="bg-[#FAF7F2] text-[#59463B] border-[#D6CCC2]">{scheme.category || 'Welfare'}</Badge>
              {scheme.state && <Badge variant="outline">{scheme.state}</Badge>}
            </div>
            
            <h1 className="text-2xl md:text-3xl font-extrabold text-[#2D231E] mb-3">
              {scheme.title}
            </h1>
            ...
            <div className="flex items-center gap-3 mt-5 pt-4 border-t border-[#D6CCC2]">
              <Button variant="outline" onClick={() => toggleSaveScheme(scheme.id, scheme.title)} className={`gap-2 ${saved ? 'text-amber-600 border-amber-300 bg-amber-50' : ''}`}>
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {scheme.benefits?.map((benefit, i) => (<div key={i} className="flex items-center gap-2.5 p-3.5 bg-green-50 rounded-xl text-green-900 text-sm font-medium border border-green-200/50">
          ...
          <section>
            ...
            <Card className="p-5 space-y-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="font-semibold text-[#2D231E] text-sm">Age Requirement</p>
                  <p className="text-xs text-[#7D6E63]">Applicable for Indian citizens aged 18–35 years. (Your age: {user?.age || 23} yrs ✓)</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3">
                <CheckCircle2 className="w-5 h-5 text-green-500 shrink-0 mt-0.5"/>
                <div>
                  <p className="font-semibold text-[#2D231E] text-sm">Target Groups</p>
                  <p className="text-xs text-[#7D6E63]">Students, young entrepreneurs, micro-enterprises, and skill program participants.</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3">
                <CheckCircle2 className="w-5 h-5 text-green-500 shrink-0 mt-0.5"/>
                <div>
                  <p className="font-semibold text-[#2D231E] text-sm">Geographical Scope</p>
                  <p className="text-xs text-[#7D6E63]">All States and Union Territories across India.</p>
                </div>
              </div>
            </Card>
          </section>
          ...
          <section>
            ...
            <Card className="p-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                <div className="flex items-center justify-between p-2.5 bg-[#FAF7F2] rounded-lg">
                  <span className="flex items-center gap-2">
                    <FileText className="w-4 h-4 text-[#59463B]"/> Aadhaar Card
                  </span>
                  <span className="text-xs text-green-600 font-semibold">Ready in Vault</span>
                </div>
                <div className="flex items-center justify-between p-2.5 bg-[#FAF7F2] rounded-lg">
                  <span className="flex items-center gap-2">
                    <FileText className="w-4 h-4 text-[#59463B]"/> PAN Card
                  </span>
                  <span className="text-xs text-green-600 font-semibold">Ready in Vault</span>
                </div>
                <div className="flex items-center justify-between p-2.5 bg-[#FAF7F2] rounded-lg">
                  <span className="flex items-center gap-2">
                    <FileText className="w-4 h-4 text-[#59463B]"/> Bank Account Passbook
                  </span>
                  <span className="text-xs text-green-600 font-semibold">Ready in Vault</span>
                </div>
                <div className="flex items-center justify-between p-2.5 bg-[#FAF7F2] rounded-lg">
                  <span className="flex items-center gap-2">
                    <FileText className="w-4 h-4 text-[#59463B]"/> Domicile / Residence Proof
                  </span>
                  <span className="text-xs text-[#7D6E63]">Optional</span>
                </div>
              </div>
            </Card>
          </section>
        ...
        ].map((s) => (<div key={s.step} className="flex items-start gap-3.5 p-3.5 bg-white rounded-xl border border-[#D6CCC2]">
                  <div className="w-7 h-7 rounded-full bg-[#59463B] text-white flex items-center justify-center font-bold text-xs shrink-0 mt-0.5">
                    {s.step}
                  </div>
                  <div>
                    <h4 className="font-bold text-sm text-[#2D231E]">{s.title}</h4>
                    <p className="text-xs text-[#7D6E63] mt-0.5">{s.desc}</p>
                  </div>
                </div>))}
            </div>
          </section>
          ...
          <section>
            <h2 className="text-xl font-bold text-[#2D231E] mb-3 flex items-center gap-2">
              <HelpCircle className="w-5 h-5 text-[#59463B]"/>
              Frequently Asked Questions
            </h2>
            <div className="space-y-3">
              <Card className="p-4">
                <h4 className="font-bold text-sm text-[#2D231E] mb-1">
        ...
        <div className="lg:w-[360px] space-y-6">
          ...
          <Card className="p-6 border-2 border-[#A67C65] shadow-md space-y-4">
            <div>
              <Badge className="bg-[#FAF7F2] text-[#59463B] border-[#D5BDAF] text-[10px] mb-2">
            ...
            <Button className="w-full bg-[#A67C65] hover:bg-[#8C644F] text-white font-bold text-sm py-3 h-auto gap-2" onClick={() => window.open(scheme.portalUrl || scheme.officialUrl || 'https://www.myscheme.gov.in', '_blank')}>
              Open Official Portal <ExternalLink className="w-4 h-4"/>
            </Button>
          </Card>

          {/* Application Deadline */}
          {scheme.deadline && (<Card className="p-5 flex items-center gap-3">
              <Calendar className="w-8 h-8 text-amber-500 shrink-0"/>
              <div>
                <p className="text-xs text-[#7D6E63]">Application Deadline</p>
          ...
          {/* Official Helpline */}
          <Card className="p-5 text-xs text-[#6B5E55] space-y-2">
            <h4 className="font-bold text-[#2D231E] flex items-center gap-1.5">
              <Shield className="w-4 h-4 text-[#59463B]"/> Need Assistance?
            </h4>
            <p>National Citizen Toll-Free Desk: <strong className="font-mono text-[#2D231E]">1800-11-0001</strong></p>
      ...
      <div className="mt-8 p-4 bg-amber-50 border border-amber-200 rounded-xl text-xs text-amber-800 flex items-start gap-2.5">
